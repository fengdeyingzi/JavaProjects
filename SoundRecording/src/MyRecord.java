...
public class MyRecord extends JFrame implements ActionListener{
	...
	String path = "C:\\Users\\XPasion\\Music";
	JList list;
	ArrayList<String> list_record;
...
//		fsv.getHomeDirectory();     //读取桌面路径   
		path = fsv.getDefaultDirectory().getAbsolutePath(); //这便是读取我的文档路径的方法
		...
		jp2.add(scrollPane);
		list.setMinimumSize(new Dimension(400, 32));
		list.setPreferredSize(new Dimension(380, 160));
//		scrollPane.setMinimumSize(new Dimension(400, 32));
		...
		list.setListData(list_record.toArray());
		list.addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent listselectionevent) {
				...
				if(pos>=0 && pos<list_record.size()){
					String filename = list_record.get(pos);
					playSound(path +File.separator+ filename);
				}
				}
		...
		this.setTitle("录音机 - 风的影子 20210331");
	...
	public void playSound(String path){
		...
		AudioClip sound_choose1;
		try {
			sound_choose1 = Applet.newAudioClip(sound1.toURL());
			sound_choose1.play();   //播放音效
		} catch (MalformedURLException e) {
			e.printStackTrace();
		}
//		sound_choose1.stop();  //停止播放
	}
	...
	public void save()
	{
        ...
        try {	
        	...
        	file = new File(filePath.getPath()+File.separatorChar+name);      
            AudioSystem.write(ais, AudioFileFormat.Type.WAVE, file);
            System.out.println("保存成功"+file.getPath());
            list_record.add(name);
            list.setListData(list_record.toArray());
        } catch (Exception e) {
            e.printStackTrace();
        }finally{
